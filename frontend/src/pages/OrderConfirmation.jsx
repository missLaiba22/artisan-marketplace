...
export default function OrderConfirmation() {
    ...
    function poll() {
      ordersApi
        ...
        .then((data) => {
          ...
          if (data.payment_status === "paid" || data.payment_status === "expired") {
            setStatus("ready");
            return;
          }
  ...
  if (status === "loading") {
    return (
      <div className="max-w-5xl mx-auto px-6 py-16 text-center">
        <p className="font-mono text-sm text-ink-soft">Confirming your payment…</p>
      </div>
    );
  }
  ...
  if (status === "error" || !checkout) {
    return (
      <div className="max-w-5xl mx-auto px-6 py-16 text-center">
        <p className="text-clay text-sm mb-4">Couldn't load your order. Check My Orders shortly.</p>
        <Link to="/orders" className="text-brass font-medium hover:underline">
          Go to My Orders
        </Link>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto px-6 py-12 sm:py-16">
      {isPending && (
        <div className="bg-brass-soft/30 border border-brass/30 rounded-2xl px-5 py-4 mb-8 text-sm">
          Payment is still confirming — this can take a few seconds. Refresh this page
          or check <Link to="/orders" className="underline font-medium">My Orders</Link> shortly.
        </div>
      )}
      ...
      <section>
        ...
        <p className="text-ink-soft text-sm sm:text-base mb-6 max-w-2xl leading-relaxed">
          {isPaid
            ? `Payment confirmed — split into ${orderCount} shop ${orderCount === 1 ? "order" : "orders"}.`
        ...
        <div className="bg-white/70 border border-ink/10 rounded-3xl p-5 sm:p-6 shadow-[0_12px_30px_rgba(66,40,25,0.06)] mb-6">
          {checkout.orders.map((order) => (
            <div key={order.id} className="border-b border-ink/10 last:border-0 py-4 first:pt-0 last:pb-0">
