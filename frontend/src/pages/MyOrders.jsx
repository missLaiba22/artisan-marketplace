      ...
      <div className="flex flex-wrap items-start justify-between gap-4 px-5 py-4 border-b border-ink/10 bg-parchment/50">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <PaymentStatusBadge status={checkout.payment_status} />
          </div>

          <h2 className="font-display text-2xl mt-1">
            {checkout.payment_reference ?? checkout.id.slice(0, 8)}
          </h2>

          <p className="text-sm text-ink-soft mt-2">
            {orderCount} shop {orderCount === 1 ? "order" : "orders"} ·{" "}
        ...
        <div className="text-right">
          ...
          <p className="font-mono text-xl text-brass mt-1">
      ...
      <div className="p-5 space-y-4">
        ...
        {!expanded && checkout.orders.length > 2 && (
          <p className="text-sm text-ink-soft">
            Showing the first 2 shop orders from this checkout.
          </p>
        )}
      </div>
    </article>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-ink-soft text-sm sm:text-base max-w-2xl leading-relaxed">
            Review every checkout you’ve placed, with the latest receipt at
            the top and the item breakdown saved by the backend.
          </p>
        </div>

        <Link
          to="/shop"
          className="inline-flex items-center justify-center rounded-full bg-ink text-parchment px-5 py-3 font-medium hover:bg-ink-soft transition-colors"
        >
          Continue shopping
        </Link>
      </div>
      ...
      {status === "ready" && history.length === 0 && (
        <div className="bg-white/70 border border-ink/10 rounded-3xl p-8 text-center">
          ...
          <p className="text-ink-soft text-sm mb-6">
            Your purchases will appear here after your first checkout.
          </p>

          <Link
            to="/shop"
            className="inline-flex items-center justify-center rounded-full bg-maroon text-white px-5 py-3 font-medium hover:bg-crimson transition-colors"
          >
            Browse the shop
          </Link>
        </div>
